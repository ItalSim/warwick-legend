...
void WLGDPrimaryGeneratorAction::GeneratePrimaries(G4Event* event)
{
  ...
  if(fGenerator == "OpticalMap")
    {
      ...
      //inradius of the 12-sided shield at 2m: 2m (seems kinda obvious, right?
      //circumradius of the 12-sided shield at 2.1m (includes thickness): 2.17408
      
      //For now, let the radius of the position be between 2.17408 and 2.75 (ostensibly, the inner radius of the cryostat)
      //Update: the inner radius of the cryostat is apparently 2.69, because it's defined differently than the other volumes. Whatever
      //It is smaller by (cryostat thickness x2) because of the sequential 3 cm thick cryostat walls
      //Update 14/11: reducing the inner sampling radius in order to include the entire LAr volume on the outside
      ...
      while(!correctvolume)
	{
      ...
      fParticleGun->SetParticleDefinition(tpt->FindParticle("opticalphoton"));
      fParticleGun->SetParticleMomentumDirection(G4ThreeVector(px,py,pz));
      fParticleGun->SetParticleEnergy(energy*eV);
      fParticleGun->SetParticlePosition(G4ThreeVector(x*m,y*m,z*m));
      
      //For polarization, just do the same thing we did for momentum
      //Assigning it here in order to reuse variables
      phi = CLHEP::twopi * rndm(generator);
      theta = CLHEP::pi * rndm(generator);
  ...
  if(fGenerator == "ArgonCaptureGammas")
    {
      //G4cout << "ARGON CAPTURE GAMMAS" << G4endl;
      OpenArFile();
      ...
      //single line <-> single muon, 1 or more captures/gammas
      ...
      while(iss >> num)//Line-by-line
	{
	  ...
	  //else if(!numberofgammas)//Fresh capture, clear the appropriate variables
	  //ResetCapture();
